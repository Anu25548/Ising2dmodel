...
if run_sim:
    ...
    # Magnetization vs Temperature Tab
    with tabs[0]:
...
- **Y-axis (Magnetization):** Average alignment of all spins. High value ($\\sim \\pm1$) means almost all spins point the same way *(system magnetized/ferrromagnetic)*. Near zero means no preferred direction (random, paramagnetic).
- **X-axis (Temperature):** How hot the system is. As T increases, randomness increases.
- **Red dashed line:** Theoretical critical temperature $T_c$ where phase transition occurs.

#### **Physics & Interpretation**

- **$T < T_c$ (left of red line):** The system is *ferromagnetic* (ordered). All spins tend to align, and net magnetization is high.
- **$T = T_c$ (at red line):** *Phase transition!* Magnetization drops sharply; long-range order disappears.
- **$T > T_c$ (right of red line):** The system is *paramagnetic* (disordered). Spins are random. The average net magnetization approaches zero.

#### **Why are there error bars?**

- Error bars show the uncertainty/fluctuations due to randomness (finite size and sampling). They're largest near $T_c$, where the system is most "undecided."

#### **What to notice:**

- The sharp drop at $T_c$ is the **signature of spontaneous symmetry breaking**—the hallmark of phase transitions.
- At high T, even though spins flip constantly, the *average* magnetization is close to zero.
- Compare "Experiment" and "Simulation" traces when available.
...
- **Color regions:** Blue (left) = Ferromagnetic (ordered); Orange (right) = Paramagnetic (disordered). The red dashed line marks the phase transition at $T_c$.
- **Black dots:** Show magnetization (with error bars) versus temperature on the same diagram.
- **Spin snapshots:** 
    - *Left image (below $T_c$):* Almost all spins point the same way—system is magnetized; macroscopic order is visible.
    - *Right image (above $T_c$):* Spins are random—no overall magnetization; the system has lost memory of former order.

#### **Physics & Interpretation**

- The **boundary between blue/orange** is the “phase boundary,” separating ordered and disordered regions.
- The *actual* configuration plots (below/above $T_c$) show what “order” and “disorder” really mean in the Ising model.
- Studying how the system “loses” order at $T_c$—both via curves and via direct spin patterns—is the core of statistical physics!

#### **What can you investigate here?**

- How does changing $N$ affect the sharpness of transition?
- How well do “snapshots” represent statistical order/disorder for a given T?
- Visual connection: The sudden loss of global alignment at $T_c$ is both in plot and lattice image.
...
- Shows the probability distribution of $|M|$ (magnetization per spin) at a *single* chosen temperature.
- Each bar = How often the system had a given magnetization value during MC run.

#### **Physics & Interpretation**

- **$T \\ll T_c$:** Histogram sharply peaked near $|M|=1$. System almost always aligned → *ordered phase*.
- **$T \\approx T_c$:** Often bimodal or broad—system fluctuates between order and disorder. *Signature of critical behavior.*
- **$T \\gg T_c$:** Tight, sharp peak near $|M|=0$. System is almost always disordered → *paramagnetic phase*.

#### **How is this useful?**

- Direct evidence for phase coexistence, critical fluctuations, and order parameter statistics—real research-grade information!
---
        """)
    ...
    with tabs[4]:
        ...
        st.markdown("""
---
#### **🎞️ What do the animations show?**

- **Top Animation (Spin Evolution at Fixed T):**
    - Watch how the spins “settle” over MC steps at a chosen temperature.
    - For $T < T_c$: See order grow—domains align, system self-magnetizes.
    - For $T > T_c$: See spins flip, never order—chaos reigns!

- **Bottom Animation (Temperature Sweep):**
    - See how the entire lattice “melts” from order to randomness as $T$ increases.
    - It's a direct, visual, physical demonstration of the phase transition.

#### **Physics & Usage**

- Animations make *dynamics* and *critical fluctuations* intuitively clear—not just numbers, but real movement!
- Use for classroom demos, research presentations, or self-study experiments.
- Pause/replay to see domain boundaries, spontaneous flips, cluster coalescence...
